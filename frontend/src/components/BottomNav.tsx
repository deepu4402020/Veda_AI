...
export default function BottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-4 left-4 right-4 bg-[#1C1C1D] text-gray-400 rounded-3xl pb-2 pt-2 px-6 flex justify-between items-center shadow-[0_8px_30px_rgba(0,0,0,0.4)] z-50 h-[4.5rem]">
      {navItems.map((item) => {
        ...
        return (
          <Link key={item.name} href={item.href} className="flex flex-col items-center justify-center w-16 h-full gap-1">
            <div className="relative">
              ...
              />
              {item.badge && (
                <span className="absolute -top-1 -right-2 bg-[#FF453A] text-white text-[10px] font-bold px-1 rounded-full border border-[#1C1C1D]">
                  {item.badge}
                </span>
              )}
            </div>
